...
export default function CadastroScreen({ telefone, onComplete, onBack, asModal = false }: Props) {
  ...
  const go = async () => {
    ...
    try {
      ...
      // dataNascimento vem de parseBirthDate no formato "YYYY-MM-DD" garantido.
      // Comparação direta de string funciona corretamente nesse formato
      // (lexicográfica = cronológica para YYYY-MM-DD) e evita bugs de fuso
      // horário do new Date() que variam entre browsers.
      // Corte: nascidos a partir de 01/01/2007 são menores de idade.
      if (dataNascimento >= '2007-01-01') {
        setError('É necessário ser maior de idade para participar.')
        setLoading(false)
        return
      }
